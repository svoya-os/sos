...
# Prefer the branding team's lockup for the welcome page when it exists.
for cand in "$SVOYA_SRC/branding/out/logo/svoya-lockup-stacked-en-on-dark.png"; do
    if [ -f "$cand" ]; then install -m0644 "$cand" "$f/etc/calamares/branding/svoya/welcome.png"; fi
done
