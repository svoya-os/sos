...
| `sos apps [--json]` | The app catalog by category (`data/apps.toml`: games, chat, internet, office, creative, players, code, system), with what is installed. The launcher reads `--json` for its «Установить …» rows and its install list; Jackson for «установи …». |
...
| `sos morse [text] [--wpm N] [--quiet] [--no-sound] [--json]` | Morse code (Latin and Cyrillic), played as a soft 650 Hz tone through `pw-play`/`paplay`/`aplay`; `sos morse` alone sends SOS. |
| `sos session-start` | Hyprland `exec-once` (§5): theme → export `WAYLAND_DISPLAY`/`HYPRLAND_INSTANCE_SIGNATURE` to systemd/D-Bus → jacksond → shell → first-run wizard → on the first login, Jackson's skills folder (`jackson skills init`). Idempotent, never blocks; log in `~/.local/state/svoya/session.log`. |
