...
The keys and values are those of ``jackson/jackson/avatar.py`` (tests keep the two in sync).
"""
...
def encode(data: object) -> str:
    """Look → ``character=cat;skin=snow;…`` (empty when there is nothing to say)."""
    look = clean(data)
    parts = []
    for key in KEYS:
        if key not in look:
...
def decode(spec: str) -> dict:
    ...
    for part in filter(None, spec.split(";")):
        ...
        if not sep or key not in KEYS or key in raw:
